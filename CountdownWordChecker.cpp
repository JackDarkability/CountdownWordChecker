...
bool isWord(string line, char letters[], int numOfLetters, int numDifferencesAllowed = 0)
{
    ...
    for (int i = 0; i < numOfLetters; i++)
    {
        ...
        { // character in word
            int numRemovals = 0;
            while (positionsChecked[index] != -1)
            { // If there are multiple of the same letter in the word and already checked one of them
                line.erase(index, 1);
                numRemovals++;
                index = line.find(letters[i]);
                if (line.find(letters[i]) == line.npos)
                { // If no more of that letter in word and still letter remaining in letters[]
                    isWordCheck = false;
                    delete[] positionsChecked;
                    return isWordCheck;
                }
...
int main()
{
    ...
    if (wordsFile.is_open())
    {
        ...
        {
            if (line.length() == numOfLetters)
            { // If could be whole word.
